...
class genesysCompute:
    def __init__(self, decoder, stats, testDir) -> None:
        ...
        self.simdLoadVmem1Cycles = 0
        self.simdLoadVmem2Cycles = 0
    ...
    def systolic_sim(self, mode):
        ...
        self.gStats.genesys_stats['Simd']['perTileSoftmax'] = 0
        self.gStats.genesys_stats['Simd']['simdLoadVmem1Cycles'] = 0
        self.gStats.genesys_stats['Simd']['simdLoadVmem2Cycles'] = 0
    ...
    def simd_sim(self, mode):
        ...
        config = ConfigParser(self.decoder.testPath, self.decoder.layerName, ddrBandwidth = self.decoder.infBandwidth, ddrFrequency = self.decoder.infFrequency, sim_config_path=_sim_config_path).parse()
        ...
        self.simdLoadVmem1Cycles = self.simdResult['ddrLoadCycleVmem1']
        self.simdLoadVmem2Cycles = self.simdResult['ddrLoadCycleVmem2']
        ...
        # self.gStats.genesys_stats['Genesys']['totCycles'] = ((self.simdComputeCycles + self.simdStoreCycles) * self.decoder.numSIMDComputeTiles) + \
                                                            # (self.simdLoadVmem1Cycles * self.decoder.numSIMDLoadTiles['vmem1']) + (self.simdLoadVmem2Cycles * self.decoder.numSIMDLoadTiles['vmem2'])
        
        # Initial load latency for the first tile (only load latencies for one tile)
        initial_load_latency = max(self.simdLoadVmem1Cycles, self.simdLoadVmem2Cycles)

        # Final store latency for the last tile
        final_store_latency = self.simdStoreCycles

        # Determine the maximum latency of the pipelined stages (i.e., load, compute, store) for each tile
        pipelined_stage_latency = max(self.simdComputeCycles, self.simdLoadVmem1Cycles, self.simdLoadVmem2Cycles, self.simdStoreCycles)

        # Calculate total cycles by adding initial load latency, the pipelined cycles for each tile,
        # and final store latency at the end
        self.gStats.genesys_stats['Genesys']['totCycles'] = initial_load_latency + \
                                                            (pipelined_stage_latency * (self.simdResult['numTiles'] - 1)) + \
                                                            final_store_latency
        ...
        self.gStats.genesys_stats['Simd']['perTileSoftmax'] = self.simdResult['perTileSoftmax']  # for softmax computation
        self.gStats.genesys_stats['Simd']['simdLoadVmem1Cycles'] = self.simdResult['ddrLoadCycleVmem1']
        self.gStats.genesys_stats['Simd']['simdLoadVmem2Cycles'] = self.simdResult['ddrLoadCycleVmem2']
    ...
    def genesys_sim(self, layerType, mode):
        ...
        fused = True if "fused" in self.decoder.layerType else False
        print ("fused = ", fused)
        print("isGemmlayer =", self.decoder.isGemmlayer)
        sysSim = systolic_sim(self.decoder.configPath, self.decoder.testPath, ddrBandwidth = self.decoder.infBandwidth, \
        ...
        _sim_config_path = os.path.join(self.decoder.configPath, 'simd_config.json')
        # print (" ------- In SIMD Sim")
        config = ConfigParser(self.decoder.testPath, self.decoder.layerName, ddrBandwidth = self.decoder.infBandwidth, ddrFrequency = self.decoder.infFrequency, sim_config_path=_sim_config_path, layerType = layerType).parse()
        pipeline = Pipeline(config)
        self.simdResult, self.simdBufAccesses = pipeline.run(inst_file_path=config["instructions_path"])
    ...
    def getSIMDCycles(self, tilenum):
        if tilenum < max(self.decoder.numSIMDLoadTiles['vmem1'], self.decoder.numSIMDLoadTiles['vmem2']):
            _simdload = self.simdLoadVmem1Cycles + self.simdLoadVmem2Cycles
        else:
            _simdload = 0
    ...
    def computeTile(self, _arr1DIndex, _prevArr1DIndex, _iterCount):
        ...
        if _iterCount == -1:
            ...
            print ("Here 2 _totalCyclesThisIter", _totalCyclesThisIter, "load cycle = ", _ibufLoadCycles, _wbufLoadCycles,  'compute = ', _computeCycles)
            ...
            print ("Here 3 _totalCyclesThisIter", _totalCyclesThisIter, "_simdStoreCycles", _simdStoreCycles, "_sysComputeCycles", _sysComputeCycles )
            ...
            _storeCycles = self.simdStoreCycles
            _totSIMDcycles = _simdStoreCycles
            print ("Here 5 _totalCyclesThisIter", _totalCyclesThisIter, "_simdStoreCycles", _simdStoreCycles, "_wbufLoadCycles",_wbufLoadCycles , "_syscomputeCycles", _computeCycles, "simd compute ", _simdComputeCycles)
    ...
    def get_energy(self):
        ...
        #print ('here' , type(self.simdBufAccesses))
        #print ('here 1 ', self.simdBufAccesses)
        if len(self.simdBufAccesses) > 0:            
            vmem1 = True if self.simdBufAccesses['vmem1_computeRead'] + self.simdBufAccesses['vmem1_computeWrite'] > 0 else False
            vmem2 = True if self.simdBufAccesses['vmem2_computeRead'] + self.simdBufAccesses['vmem2_computeWrite'] > 0 else False
    ...
    def compile_energy_stats(self, stats):
        ...
        self.gStats.genesys_stats['Energy']['vmem1_numTile'] =  stats['vmem1']['numTiles']  # DEBUG
        ...
        self.gStats.genesys_stats['Energy']['vmem1_totalNumReads'] =  stats['vmem1']['totalNumReads']  
        self.gStats.genesys_stats['Energy']['vmem1_totalReadEnergy'] =  stats['vmem1']['totalReadEnergy']  
        self.gStats.genesys_stats['Energy']['vmem1_totalNumWrites'] =  stats['vmem1']['totalNumWrites']  
        self.gStats.genesys_stats['Energy']['vmem1_totalWriteEnergy'] =  stats['vmem1']['totalWriteEnergy']  
        ...
        self.gStats.genesys_stats['Energy']['vmem2_totalNumReads'] =  stats['vmem2']['totalNumReads']  
        self.gStats.genesys_stats['Energy']['vmem2_totalReadEnergy'] =  stats['vmem2']['totalReadEnergy']  
        self.gStats.genesys_stats['Energy']['vmem2_totalNumWrites'] =  stats['vmem2']['totalNumWrites']  
        self.gStats.genesys_stats['Energy']['vmem2_totalWriteEnergy'] =  stats['vmem2']['totalWriteEnergy']  
